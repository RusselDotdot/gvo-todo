<template>
    <div class="board" @drop="onBoardDrop($event, position)" @dragenter.prevent @dragover.prevent>
        ...
        <div class="card-group">
            ...
            <div class="task-list">
                <AppTask v-for="task in newTasks" :task="task" :draggable="isDraggable" :key="`newTasks-${task.title}`" @drag-start="startDrag" @task-drop="onTaskDrop" :class="isDraggable? 'draggable' : ''"/>
            </div>
        </div>
    </div>
</template>
